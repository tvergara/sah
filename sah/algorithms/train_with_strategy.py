...
class TrainWithStrategy(LightningModule):
    def __init__(
        ...
        self.genera_config = general_config
    ...
    def build_pretraining_dataset(self):
        ...
        self.pretraining_dataset = ConcatDataset(self.variant_datasets)
        budget = self.general_config.pretraining_budget
        if budget is not None and budget < len(self.pretraining_dataset):
            idx = torch.randperm(len(self.pretraining_dataset))[:budget].tolist()
            self.pretraining_dataset = Subset(self.pretraining_dataset, idx)
