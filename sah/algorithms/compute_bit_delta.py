...
class SyntheticDataset(Dataset):
    def __init__(self, teacher_model, vocab_size: int, seq_length: int, dataset_size: int = 4000):
...
class ComputeMask(LightningModule):
    def __init__(
        self,
        ...
        distilled_config: CheckpointConfig,
        seq_length: int = 100,
        ...
        self.objective = hydra_zen.instantiate(transformer_config, vocab_size=self.tokenizer.vocab_size)
        # breakpoint()
        load_weights_from_checkpoint(self.objective, distilled_config.path, model_name='model')

        # Store hyperparameters
        self.seq_length = seq_length
        self.dataset_size = dataset_size
        self.batch_size = batch_size
        self.diffs = [(p_fine - p_pre).to('cuda') for p_fine, p_pre in zip(self.objective.parameters(), self.model.parameters())]
    ...
    def training_step(self, batch, batch_idx):
        ...
        diffs = [p_fine - p_pre for p_fine, p_pre in zip(self.model.parameters(), self.model_copy.parameters())]
        non_ignorable = [(torch.abs(pd) > 0.005) for pd in self.diffs]
        positives = sum(((pg * pd * ig.float()) > 0).sum() for pg, pd, ig in zip(diffs, self.diffs, non_ignorable))
        non_ig = sum([p.float().sum() for p in non_ignorable])
        self.log('positives', positives.item() / non_ig.item())
        return loss
