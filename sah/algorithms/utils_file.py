...
def load_weights_from_checkpoint(model, path, model_name='network'):
    ...
    for k, v in state_dict.items():
        if k.startswith(f"{model_name}.") and 'scale' not in k:
            k = k.replace('original_weight', 'weight').replace('original_bias', 'bias').replace('.layer.', '.')
            name = k[len(model_name) + 1:]
            new_state_dict[name] = v

    model.load_state_dict(new_state_dict)

def load_lora_from_checkpoint(model, config, model_name='model'):
    ...
    ckpt = torch.load(config.path + '.ckpt', map_location=lambda storage, loc: storage)
    state_dict = ckpt["state_dict"]
