...
# Use monkeypatch.setattr(sah.utils.remote_launcher_plugin, ..., that_mock)
...
import sah.main
import sah.utils.remote_launcher_plugin
from sah.main import PROJECT_NAME, main
from sah.main_test import CONFIG_DIR
from sah.utils import remote_launcher_plugin
from sah.utils.env_vars import SLURM_JOB_ID
from sah.utils.remote_launcher_plugin import RemoteSlurmLauncher
from sah.utils.testutils import IN_GITHUB_CI, IN_SELF_HOSTED_GITHUB_CI
...
def test_run_remote_job(argv: list[str], monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        sah.utils.remote_launcher_plugin, RemoteSlurmLauncher.__name__, launcher_mock
    )

    monkeypatch.setattr(sys, "argv", ["sah/main.py"] + argv)
