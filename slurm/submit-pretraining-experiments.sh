...
for seed in "${SEEDS[@]}"; do
  for lr in "${LR_ONLINE_CODING[@]}"; do
    for max_ex in "${MAX_EXAMPLES[@]}"; do
      ...
      sbatch --gres=gpu:a100l:1 slurm/run-experiment.sh olmo3-32b-step656k metamath online-coding seed=$seed algorithm.strategy.lr=$lr algorithm.max_examples=$max_ex algorithm.strategy.ft_strategy=qlora algorithm.batch_size=2 trainer.accumulate_grad_batches=4
    done
  done
done
...
for seed in "${SEEDS[@]}"; do
  for lr in "${LR_ONLINE_CODING[@]}"; do
    for max_ex in "${MAX_EXAMPLES[@]}"; do
      ...
      sbatch --gres=gpu:a100l:1 slurm/run-experiment.sh olmo3-32b-step656k flores online-coding seed=$seed algorithm.strategy.lr=$lr algorithm.max_examples=$max_ex algorithm.strategy.ft_strategy=qlora algorithm.batch_size=2 trainer.accumulate_grad_batches=4
    done
  done
done
